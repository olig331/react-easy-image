...
export const AnimatedSlides = React.forwardRef((props, ref) => {
    ...
    function sortImages() {
        if (typeof (props.images[0].img) === 'object') {
            for (let i = 0; i < props.images.length; i++) {
                let copy = props.images[i];
                copy.img = props.images[i].img.default
                appliedImages.push(copy);
            }
        } else {
            appliedImages = [...props.images]
        }
    }
    sortImages();
    ...
    const applyConfig = {
        imgWidth: props.userConfig.imgWidth,
        imgHeight: props.userConfig.imgHeight,
        dotHighlightColor: props.userConfig.dotHighlightColor ?? "violet",
        dotBgColor: props.userConfig.dotBgColor ?? "#777",
        maxShadowBlur: props.userConfig.maxShadowBlur ?? 64,
        shadowColor: props.userConfig.shadowColor ?? "rgba(0,0,0, 0.19)",
        widthNumForCalc: parseInt(props.userConfig.imgWidth.replace(/([a-z]|[A-Z]|[%])/g, "")),
        heightNumForCalc: parseInt(props.userConfig.imgHeight.replace(/([a-z]|[A-Z]|[%])/g, "")),
        rotateTime: props.userConfig.rotateTime ?? 350,
        borderRadius: props.userConfig.borderRadius ?? "5px",
        capColor: props.userConfig.capColor ?? "#333",
        capBgColor: props.userConfig.capBgColor ?? "rgba(220,220,220, 0.89)",
        capFontSize: props.userConfig.capFontSize ?? "14px"
    };

    // React State 
    const [indexes, setindexes] = useState([2, 3, 4]);
    const [currentNumber, setcurrentNumber] = useState(() => Math.round(props.images.length / 2))
    ...
    const calculateNewIndexes = (index) => {
        let arr = new Array(3);
        arr[0] = index == 0 ? props.images.length - 1 : index - 1;
        arr[1] = index;
        arr[2] = index == props.images.length - 1 ? 0 : index + 1;
        return arr;
    }
    ...
    for (const [index, dot] of props.images.entries()) {
        dots.push(
            <span
                key={index}
    ...
    const rotateBack = () => {
        ...
        setcurrentNumber(prev => prev === 0 ? props.images.length - 1 : prev - 1)
    }
    ...
    const rotateForward = () => {
        ...
        setcurrentNumber(prev => prev === props.images.length - 1 ? 0 : prev + 1)
    }
    ...
    const caroselLogic = {
        offset_index: function (image_index, offset) {
            return ((image_index + offset) + props.images.length) % props.images.length
        },
         ...
         `}>
            ...
            >
                ...
                <img
                    ...
                    onLoad={loadTimes < 1 ? handleFirstRenderImgHeight : null}
                    src={
                        iteration == 0
                            ? appliedImages[indexes[1]].img
                            : iteration === 1
